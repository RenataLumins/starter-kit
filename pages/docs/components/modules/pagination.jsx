...
function PaginationComponentsPage() {
	...
	return (
		<div className="ui-typography">
			...
			<table className="tw-w-full ui-table">
				<tbody>
					{Array.from(Array(singersPerPage), (e, index) => (
						<tr key={index}>
							<td className="tw-w-[60px]">{(index + 1) + (singersPerPage * (currentPage - 1))}</td>
							<td>{Singers[index + (singersPerPage * (currentPage - 1))]['name']}</td>
							<td className="tw-w-[200px]">{Singers[index + (singersPerPage * (currentPage - 1))]['country']}</td>
						</tr>
					))}
				</tbody>
			</table>
