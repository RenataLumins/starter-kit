	...
	return (
		<>
			<div className="ui-typography">
				...
				<div>
					<button type="button" onClick={() => setModal("local-example")}>Open local modal</button>
				</div>
				<p>Вариант 3: локальное модальное окно, с альтернативным оформлением</p>
				<div>
					<button type="button" onClick={() => setModal("local-example-2")}>Open local modal 2</button>
				</div>
			</div>

			{modal == 'local-example' && (
				<ModalOverlay isOpen={modal == 'local-example'} setOpen={setModal} className="ui-overlay-primary">
					<ModalLocalExample />
				</ModalOverlay>
			)}
			{modal == 'local-example-2' && (
				<ModalOverlay isOpen={modal == 'local-example-2'} setOpen={setModal} className="tw-bg-black tw-bg-opacity-80 tw-text-white">
					<div className="tw-relative tw-z-50 tw-pointer-events-auto">
					Example alert
					</div>
				</ModalOverlay>
			)}
