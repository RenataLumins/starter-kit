...
function ApiJSPage() {
	...
	return (
		<div className="ui-typography">
			...
			<p>Async (асинхронность) - показывает, что функция МОЖЕТ выполняться самостоятельно, и код не должен ожидать ее выполнения (в нашем случае - возвращение данных)</p>
			<p>Если функция выполняется асинхронно и, запрашивая её результат, она еще не выполнилась - вместо данных она вернет Promise</p>
			<p>Асинхронность часто  используется тогда, когда запрос к АПИ может выполняться долго, и в интерфейсе заместо результата можно показать спиннер</p>
			<br />
			<p>Если в коде нужно дождаться завершения асинхронной функции прежде, чем идти дальше - перед её вызовом нужно написать слово <b>await</b></p>
